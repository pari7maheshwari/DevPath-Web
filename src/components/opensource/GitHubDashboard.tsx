...
export default function GitHubDashboard({ accessToken }: GitHubDashboardProps) {
  ...
  const fetchIssues = async (repo: Repo) => {
    ...
    try {
      const res = await fetch(
        `https://api.github.com/repos/${repo.full_name}/issues?state=all&per_page=5`,
        {
          headers: {
            Authorization: `Bearer ${accessToken}`,
            Accept: 'application/vnd.github.v3+json',
        ...
        {selectedRepo ? (
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-2">
                {loadingIssues ? (
                  ...
                  issues.map((issue) => {
                    const isClosed =
                      issue.state === 'closed' || Boolean(issue.closed_at);
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-center gap-3 overflow-hidden">
                          ...
                          <span
                            className={`text-sm font-medium truncate ${isClosed ? 'text-muted-foreground' : 'text-foreground'}`}
                          >
                        ...
                        <Link
                          ...
                          target="_blank"
                          rel="noopener noreferrer"
                          className="text-xs text-muted-foreground hover:text-primary whitespace-nowrap ml-2"
                        >
                          View
                        </Link>
                      </div>
          ...
          <div className="h-full flex flex-col items-center justify-center p-8 text-center border border-dashed border-border rounded-xl text-muted-foreground">
            ...
            <p className="max-w-xs">
              Click on a repository from the list to view details and manage
              issues.
            </p>
          </div>
        )}
