    ...
    <div className="w-full max-w-4xl mx-auto mt-24 space-y-8">
      ...
      <div className="rounded-2xl border border-white/10 bg-black/40 backdrop-blur-xl overflow-hidden p-6 space-y-4">
        ...
        <ul className="space-y-2 text-slate-400 text-sm list-disc pl-5">
          <li>
            <span className="text-green-400 font-medium">+2 points</span> - Solo
            Participation
          </li>
          <li>
            <span className="text-red-400 font-medium">-1 point</span> - For
            having an Advanced commit on 20th January
          </li>
          <li>
            <span className="text-white font-medium">Team Ecosage</span> and{' '}
            <span className="text-white font-medium">AidLedger</span> got{' '}
            <span className="text-green-400 font-medium">+2 points</span> for
            their Exceptional Video and PPT based explantion from our Content
            Head of Community.
          </li>
          <li>
            <span className="text-green-400 font-medium">+1 point</span> to{' '}
            <span className="text-white font-medium">Tiya</span> for having a
...
function RankingRow({ item }: { item: Ranking }) {
  ...
  return (
    <div className={`group transition-colors ${bgHighlight} hover:bg-white/5`}>
      ...
      >
        <div
          className={`col-span-2 md:col-span-1 text-center font-bold text-lg ${rankColor}`}
        >
          #{item.rank}
        </div>
        ...
        <div className="col-span-4 md:col-span-4 text-right flex items-center justify-end gap-3 text-sm">
          <span className="font-mono text-slate-300 hidden sm:block">
            {item.total} pts
          </span>
          <button
        ...
        {isOpen && (
          ...
          >
            <div className="p-4 md:p-6 grid grid-cols-2 lg:grid-cols-5 gap-3">
              <ScoreCard
                label="Originality"
                score={item.details.originality}
                icon={<Lightbulb size={16} />}
              />
              <ScoreCard
                label="Technical"
                score={item.details.technical}
                icon={<Monitor size={16} />}
              />
              <ScoreCard
                label="Design"
                score={item.details.design}
                icon={<Award size={16} />}
              />
              <ScoreCard
                label="Innovation"
                score={item.details.innovation}
                icon={<Zap size={16} />}
              />
              <ScoreCard
                label="Presentation"
                score={item.details.presentation}
                icon={<Play size={16} />}
              />
            </div>
          </motion.div>
