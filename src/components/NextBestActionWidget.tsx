...
export default function NextBestActionWidget() {
  ...
  return (
    <section
      ...
      className="w-full rounded-2xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 p-6"
    >
      ...
      {recs.length === 0 ? (
        ...
        <div className="flex flex-col gap-3 sm:grid sm:grid-cols-2 lg:grid-cols-3">
          {recs.map((rec, i) => (
            <article
              key={rec.id}
              className="flex flex-col gap-2 p-4 rounded-xl border border-gray-100 dark:border-gray-800 bg-gray-50 dark:bg-gray-800 hover:bg-white dark:hover:bg-gray-750 hover:border-indigo-200 dark:hover:border-indigo-700 hover:shadow-sm transition-all duration-150"
              style={{ animationDelay: `${i * 80}ms` }}
            >
              <div className="flex items-center justify-between">
                <span className="text-2xl">{ICON_MAP[rec.icon] ?? '📌'}</span>
                <span className="text-xs font-medium px-2 py-0.5 rounded-md bg-indigo-50 dark:bg-indigo-950 text-indigo-700 dark:text-indigo-300">
                  {rec.tag}
                </span>
              </div>
              ...
              <Link
                href={rec.href}
                className="inline-flex items-center gap-1 text-xs font-medium text-indigo-600 dark:text-indigo-400 hover:underline underline-offset-2 mt-1"
                aria-label={rec.cta}
              >
                {rec.cta} →
              </Link>
