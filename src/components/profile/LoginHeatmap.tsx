...
export default function LoginHeatmap({ loginDates = [] }: LoginHeatmapProps) {
  ...
  const { currentStreak, maxStreak } = useMemo(
    () => calculateStreak(safeLoginDates),
    [safeLoginDates]
  );
    ...
    <div className="w-full p-4 bg-card border border-border rounded-xl">
      <div className="flex justify-between items-center mb-4">
        ...
        <div className="flex gap-4 text-xs text-muted-foreground">
          <span>
            Current Streak:{' '}
            <strong className="text-foreground">{currentStreak} days</strong>
          </span>
          <span>
            Longest Streak:{' '}
            <strong className="text-foreground">{maxStreak} days</strong>
          </span>
        </div>
      </div>
      <TooltipProvider>
          ...
          {days.map((date) => {
            ...
            return (
              <Tooltip key={date}>
                ...
                <TooltipContent>
                  <p>
                    {date}: {isLoggedIn ? 'Logged In' : 'No Activity'}
                  </p>
                </TooltipContent>
              </Tooltip>
            );
