...
export default function Rewards({
    ...
    const checkAndSync = async () => {
      ...
      try {
        ...
        if (newBadges.length > 0) {
          ...
          const newBadgeXp = newBadges.reduce(
            (sum, id) => sum + getBadgeXp(id),
            0
          );
          ...
          alert(
            `🎉 You earned ${newBadges.length} new ${label} and ${newBadgeXp} XP!`
          );
        } else {
  ...
  const BADGES = [
    ...
    },
    {
      ...
      description: 'Shared 3+ projects.',
      icon: '🏗️',
    },
    {
      id: 'builder-5',
      name: 'Architect',
      description: 'Shared 5+ projects.',
      icon: '🏢',
    },
    {
      id: 'builder-10',
      name: 'Master Builder',
      description: 'Shared 10+ projects.',
      icon: '🏰',
    },
    {
      id: 'connector-social',
    ...
    },
    {
      id: 'social-linkedin',
      name: 'Professional',
      description: 'Linked LinkedIn account.',
      icon: '💼',
    },
    {
      id: 'social-instagram',
      name: 'Socialite',
      description: 'Linked Instagram account.',
      icon: '📸',
    },
    {
      id: 'storyteller',
    ...
    },
    {
      id: 'local-hero',
      name: 'Local Hero',
      description: 'Added location details.',
      icon: '📍',
    },
    {
      id: 'streak-7',
    ...
    },
    {
      id: 'rising-star',
      name: 'Rising Star',
      description: 'Got 20+ stars on a project.',
      icon: '⭐',
    },
    {
      id: 'top-collaborator',
    ...
    <div className="space-y-8">
      ...
      <div className="bg-card border border-border rounded-xl p-6">
        <h3 className="text-xl font-bold mb-6 flex items-center gap-2">
          <CheckCircle className="text-blue-500" size={24} /> Badges &
          Milestones
        </h3>
          ...
          <div className="flex overflow-x-auto pb-6 gap-4 snap-x snap-mandatory scrollbar-thin scrollbar-thumb-primary/20 scrollbar-track-transparent hover:scrollbar-thumb-primary/40 transition-colors">
            {BADGES.map((badge) => {
              ...
              return (
                <div
                  key={badge.id}
                  className={`flex-none w-40 p-4 rounded-xl border flex flex-col items-center text-center gap-3 transition-all snap-start ${
                    isUnlocked
                ...
                >
                  <div
                    className={`text-4xl p-4 rounded-full ${isUnlocked ? 'bg-primary/10' : 'bg-muted'}`}
                  >
                    {badge.icon}
                  </div>
                  <div>
                    <h4
                      className={`font-bold text-sm ${isUnlocked ? 'text-foreground' : 'text-muted-foreground'}`}
                    >
                      {badge.name}
                    </h4>
