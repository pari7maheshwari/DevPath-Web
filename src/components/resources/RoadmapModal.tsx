...
export function RoadmapModal({ isOpen, onClose, roadmap }: RoadmapModalProps) {
    ...
    const fetchProgress = async () => {
      if (isOpen && roadmap && user) {
        try {
          ...
          const roadmapId =
            roadmap.id || roadmap.title.toLowerCase().replace(/\s+/g, '-');
          ...
          showError(
            'Unable to load your roadmap progress. Please try again.',
            6000
          );
          if (isMounted) {
            setCurrentPhaseIndex(0);
  ...
  useEffect(() => {
    ...
    const saveProgress = async () => {
      try {
        ...
        showError(
          'Unable to save your progress. Your changes may be lost.',
          6000
        );
      }
    };
  ...
  const handleComplete = async () => {
    try {
      ...
      showError(
        'Failed to save completion progress. Your XP may not be awarded.',
        6000
      );
    }
