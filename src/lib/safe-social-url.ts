...
export const sanitizeSocialUrl = (
  ...
  try {
    ...
    throw new Error(
      `${SOCIAL_LABELS[platform]} URL must be a valid https:// URL.`
    );
  }
  ...
  if (
    url.protocol !== 'https:' ||
    !ALLOWED_SOCIAL_HOSTS[platform].includes(hostname)
  ) {
    throw new Error(
      `${SOCIAL_LABELS[platform]} URL must use https:// and match ${ALLOWED_SOCIAL_HOSTS[platform][0]}.`
    );
  }
