...
const ToastMessage = ({
  ...
  let gradient =
    'radial-gradient(circle at 0% 0%, rgba(6,182,212,0.15) 0%, transparent 70%)';
  ...
  if (n.type === 'achievement') {
    ...
    gradient =
      'radial-gradient(circle at 0% 0%, rgba(245,158,11,0.15) 0%, transparent 70%)';
    ...
    gradient =
      'radial-gradient(circle at 0% 0%, rgba(16,185,129,0.15) 0%, transparent 70%)';
  }
  ...
  const addXp = (
    ...
    if (user && shouldPersist) {
      awardPoints(amount).catch((err) => {
        ...
        showError(
          'Failed to save XP. Your progress may not be persisted.',
          6000
        );
      });
    }
    ...
    const newNotifications = [
      {
        id: baseId,
        title:
          type === 'achievement' ? 'Achievement Unlocked' : `+${amount} XP`,
        subtitle: reason,
        type,
      },
    ];
