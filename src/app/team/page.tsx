    ...
    >
      <motion.article
        ...
        transition={
          shouldReduceMotion
            ? { duration: 0 }
            : { duration: 0.35, delay: (index % 12) * 0.03 }
        }
      ...
      >
        {!imageReady && (
          <div
            className="absolute inset-0 z-10 animate-pulse bg-slate-400/25"
            aria-hidden="true"
          />
        )}
        ...
        <div className="absolute bottom-0 left-0 right-0 p-3">
          ...
          <p className="text-cyan-100/80 text-[11px] leading-tight truncate">
    ...
    >
      <motion.article
        ...
        transition={
          shouldReduceMotion
            ? { duration: 0 }
            : { duration: 0.28, delay: (index % 6) * 0.04 }
        }
      ...
      >
        <div className="relative h-48 sm:h-52 bg-surface-deep overflow-hidden">
          ...
          {member.image ? (
            ...
            <div className="absolute inset-0 bg-gradient-to-br from-sky-400 to-indigo-500 flex items-center justify-center">
              <span className="text-5xl sm:text-6xl font-semibold text-black/90 tracking-tight">
                {getInitials(member.name)}
              </span>
            </div>
          )}
        ...
        <div className="p-3 sm:p-4">
          ...
          <p className="text-cyan-300 text-sm font-medium mt-1 truncate">
            {member.role}
          </p>
          {member.subRole && (
            <p className="text-cyan-100/70 text-xs mt-1 truncate">
              {member.subRole}
            </p>
          )}
      ...
      <section className="min-h-screen bg-surface-deep">
        <div className="relative overflow-hidden">
          <div className="absolute inset-0 pointer-events-none">
            ...
            >
              <path
                d="M-100 550 C 180 400, 420 640, 720 470 C 910 360, 1070 420, 1310 300"
                fill="none"
                stroke="rgba(69,224,255,0.35)"
                strokeWidth="2"
              />
              <path
                d="M-80 590 C 220 430, 450 680, 760 510 C 940 410, 1110 460, 1330 350"
                fill="none"
                stroke="rgba(44,194,255,0.22)"
                strokeWidth="2"
              />
              <path
                d="M-40 630 C 260 470, 500 720, 810 550 C 980 450, 1160 500, 1360 400"
                fill="none"
                stroke="rgba(24,173,255,0.16)"
                strokeWidth="2"
              />
            </svg>
          </div>
          ...
          <div className="container mx-auto max-w-[1320px] px-4 pt-6 md:pt-8 pb-16 md:pb-20 relative z-10">
            <div className="grid lg:grid-cols-12 gap-8 xl:gap-10 items-start lg:order-none order-2">
              <div className="lg:col-span-8 xl:col-span-8 lg:order-none order-2 pt-12 lg:pt-16">
                <p className="text-cyan-200/80 text-sm uppercase tracking-[0.2em]">
                  DevPath Community
                </p>
                ...
                <div className="mt-8 flex flex-wrap gap-3 text-xs">
                  <span className="px-3 py-1.5 rounded-full border border-white/20 text-white/85 bg-white/5">
                    {teamMembers.length} contributors
                  </span>
                  <span
                    className={`px-3 py-1.5 rounded-full border ${rolePalette['Core Admin']}`}
                  >
                    {coreAdmins.length} core admins
                  </span>
                  <span
                    className={`px-3 py-1.5 rounded-full border ${rolePalette['Head']}`}
                  >
                    {heads.length} heads
                  </span>
                  <span
                    className={`px-3 py-1.5 rounded-full border ${rolePalette['City Lead']}`}
                  >
                ...
                <div className="mt-10 grid grid-cols-2 sm:grid-cols-3 xl:grid-cols-4 gap-3 md:gap-4 content-start">
                  {cityLeads.map((member, index) => (
                    <TeamTile
                      ...
                      index={index}
                      stepClass={
                        leftStepPattern[index % leftStepPattern.length]
                      }
                    />
                  ))}
                </div>
                ...
                {owner ? (
                  <>
                    ...
                    <div className="text-center">
                      ...
                      <p className="text-cyan-100/85 text-sm mt-1">
                    ...
                    <div className="flex items-center justify-center gap-3">
                      {owner.socials?.github && (
                        <a
                          ...
                          className="inline-flex h-9 w-9 items-center justify-center border border-white/20 text-white/80 hover:text-white hover:border-cyan-300/60 transition-colors"
                          aria-label={`${owner.name} GitHub`}
                        >
                          <Github size={16} />
                        </a>
                      )}
                      {owner.socials?.linkedin && (
                        <a
                          ...
                          rel="noopener noreferrer"
                          className="inline-flex h-9 w-9 items-center justify-center border border-white/20 text-white/80 hover:text-white hover:border-cyan-300/60 transition-colors"
                          aria-label={`${owner.name} LinkedIn`}
                        >
                          <Linkedin size={16} />
                        </a>
                      )}
                      {owner.socials?.instagram && (
                        <a
                          ...
                          rel="noopener noreferrer"
                          className="inline-flex h-9 w-9 items-center justify-center border border-white/20 text-white/80 hover:text-white hover:border-cyan-300/60 transition-colors"
                          aria-label={`${owner.name} Instagram`}
                        >
                          <Instagram size={16} />
                        </a>
                      )}
                    ...
                    <div className="border-t border-white/10 pt-6">
                      <h3 className="text-white text-sm uppercase tracking-[0.16em] text-center">
                        Core Admins
                      </h3>
      ...
      <section className="bg-white py-20">
        <div className="container mx-auto px-4">
          <div className="max-w-4xl mx-auto text-center">
            <p className="text-sky-700/70 text-sm uppercase tracking-[0.2em]">
              Our Mission
            </p>
          ...
          <div className="mt-12 grid md:grid-cols-2 xl:grid-cols-4 gap-4">
            <ValueCard
              ...
              body="We share progress, celebrate contributions, and keep our roadmap transparent."
            />
            <ValueCard
              title="Mentor-first culture"
              body="Students and early professionals get guidance through hands-on collaboration."
            />
            <ValueCard
              title="Community at scale"
              body="City leads and role-based teams help us create impact across regions."
            />
            <ValueCard
              title="Practical outcomes"
              body="Projects, events, and pathways are designed to deliver real career growth."
            />
          </div>
          ...
          <div className="mt-10 grid md:grid-cols-2 gap-4">
            <article className="rounded-2xl border border-slate-200 p-6 bg-slate-50">
              ...
              <Link
                href="/community"
                className="inline-block mt-5 text-sm font-semibold text-sky-700 hover:text-sky-800"
              >
                Read more
              </Link>
            </article>
            ...
            <article className="rounded-2xl border border-slate-200 p-6 bg-gradient-to-br from-surface-deep to-surface-elevated text-white">
              ...
              <p className="mt-2 text-cyan-50/80 text-sm">
                We are continuously expanding with volunteer and leadership
                roles in multiple cities.
              </p>
              <Link
                href="/signup"
                className="inline-block mt-5 text-sm font-semibold text-cyan-200 hover:text-cyan-100"
              >
                View openings
              </Link>
            </article>
