...
const FALLBACK_TOP: TopContributor[] = [
  ...
  },
  {
    ...
    handle: '@schrodingerspet',
    contributions: 300,
    rank: 2,
    avatar: null,
  },
  {
    name: 'Niteshagarwal01',
    handle: '@Niteshagarwal01',
    contributions: 150,
    rank: 3,
    avatar: null,
  },
    ...
    async function fetchContributorsData() {
      try {
        const res = await fetch(
          'https://api.github.com/repos/devpathindcommunity-india/DevPath-Web/contributors'
        );
        if (!res.ok) {
        ...
        const totalContributionsCount = sorted.reduce(
        ...
        try {
          ...
          const commitsRes = await fetch(
            `https://api.github.com/repos/devpathindcommunity-india/DevPath-Web/commits?since=${thirtyDaysAgo.toISOString()}`
          );
          if (commitsRes.ok) {
    ...
    <div className={styles.container}>
      <div className={styles.hero}>
        ...
        <p className={styles.subtitle}>
          DevPath is built by developers, for developers. Thank you to everyone
        ...
        <div className={styles.podium}>
          {arrangePodium(topContributors).map((contributor) => (
            <div
              ...
              className={`${styles.podiumPlace} ${contributor.rank === 1 ? styles.first : ''}`}
            >
              <div className={styles.avatarWrapper}>
                ...
                {contributor.rank === 2 && (
                  <div
                    ...
                    style={{ fontSize: '24px', filter: 'grayscale(1)' }}
                  >
                    🥈
                  </div>
                )}
                {contributor.rank === 3 && (
                  <div
                    className={styles.crown}
                    style={{ fontSize: '24px', filter: 'sepia(1)' }}
                  >
                    🥉
                  </div>
                )}
              ...
              <div className={styles.podiumContributions}>
        ...
        {contributors.map((contributor, index) => (
          <div key={index} className={styles.card}>
            ...
            <div className={styles.types}>
              {contributor.types.includes('code') && (
                <div className={styles.typeIcon} title="Code">
                  <Code size={16} />
                </div>
              )}
              {contributor.types.includes('docs') && (
                <div className={styles.typeIcon} title="Documentation">
                  <FileText size={16} />
                </div>
              )}
              {contributor.types.includes('design') && (
                <div className={styles.typeIcon} title="Design">
                  <MessageSquare size={16} />
                </div>
              )}
              {contributor.types.includes('community') && (
                <div className={styles.typeIcon} title="Community">
                  <Github size={16} />
                </div>
              )}
            </div>
