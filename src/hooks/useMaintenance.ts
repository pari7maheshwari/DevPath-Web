...
export function useMaintenance() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(
      ...
      (doc) => {
        if (doc.exists()) {
          ...
          const rawMessage =
            ...
            'We are currently under scheduled maintenance. Please check back later.';
          ...
          const isActive =
            envOverride || modeValue === true || modeValue === 'true';
      ...
      (error) => {
        console.error(
          'Maintenance Hook: Error fetching maintenance status:',
          error
        );
