...
export const useUIStore = create<UIStore>((set, get) => ({
  ...
  syncTheme: (theme, resolvedTheme) => {
    const nextResolvedTheme: ThemePreference =
  ...
  showError: (message, duration) =>
    get().showToast(message, 'error', duration ?? 6000),
  showSuccess: (message, duration) =>
    get().showToast(message, 'success', duration ?? 4000),
  showWarning: (message, duration) =>
    get().showToast(message, 'warning', duration ?? 5000),
  showInfo: (message, duration) =>
    get().showToast(message, 'info', duration ?? 4000),
}));
...
export const useSetSearchOpen = () =>
  useUIStore((state) => state.setSearchOpen);
